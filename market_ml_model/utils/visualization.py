...
def plot_equity_curve(
    ...
    if len(equity_curve) > 1:
        ...
        plt.fill_between(
            ...
            equity_curve.values * (1 + drawdowns.values), # End fill based on drawdown %
...
def plot_drawdowns(
    ...
    for i, (_, row) in enumerate(top_drawdowns.iterrows()):
        ...
        end = row['recovery_date'] if pd.notna(row['recovery_date']) else row['end_date']
...
def plot_monthly_returns(
    ...
    monthly_pivot = monthly_returns.groupby([monthly_returns.index.year, monthly_returns.index.month]).mean().unstack(level=-1)
    ...
    sns.heatmap(
        ...
        cbar_kws={"shrink": .8} # Adjusted color bar size
    )
    ...
    plt.xticks(np.arange(len(monthly_pivot.columns)) + 0.5, month_names[:len(monthly_pivot.columns)])
