...
def calculate_returns_metrics(returns: pd.Series) -> Dict[str, float]:
    ...
    metrics["win_rate"] = len(win_days) / len(returns) if len(returns) > 0 else 0
    ...
    lose_streak_lengths = np.diff(np.append(lose_streak_ends, len(lose_streak)))

    # Get only streaks of wins/losses (not zeros)
    win_streak_lengths = (
        win_streak_lengths[win_streak.iloc[win_streak_ends] > 0]
        if len(win_streak_ends) > 0
        else np.array([])
    )
    lose_streak_lengths = (
        lose_streak_lengths[lose_streak.iloc[lose_streak_ends] > 0]
        if len(lose_streak_ends) > 0
        else np.array([])
    )

    metrics["max_consecutive_wins"] = (
        max(win_streak_lengths) if len(win_streak_lengths) > 0 else 0
    )
    metrics["max_consecutive_losses"] = (
        max(lose_streak_lengths) if len(lose_streak_lengths) > 0 else 0
    )
    ...
    if STATS_AVAILABLE:
        ...
        if len(valid_returns) > 1:
            ...
            metrics["kurtosis"] = stats.kurtosis(
                valid_returns
