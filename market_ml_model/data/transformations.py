    ...
    if fallback_source:
        for std_name, original_col in col_map.items():
            if original_col is None: # If the standard column was missing
                logger.warning(f"Missing standard column '{std_name}'. Using fallback '{fallback_source}'.")
                ohlc_std[std_name] = data[fallback_source] # Use original data for fallback
    else:
        # If neither open nor close exists, we can't really do OHLC resampling
        logger.error("Cannot perform OHLC resampling: requires at least 'open' or 'close' column.")
