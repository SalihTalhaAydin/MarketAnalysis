...
def calculate_position_size(
    ...
    if signal_strength < 0 or signal_strength > 1:
         logger.warning(f"Signal strength {signal_strength} out of bounds [0, 1]. Clamping.")
    ...
    if use_kelly:
        ...
        if win_rate is not None and payoff_ratio is not None:
            if payoff_ratio <= 1e-6: # Use epsilon comparison
                 logger.warning("Payoff ratio is zero or negative, cannot use Kelly. Defaulting to max risk.")
            ...
            kelly_payoff = 2.0
            logger.info(f"Estimating Kelly params: Win Rate={kelly_win_rate:.2f}, Payoff={kelly_payoff:.1f}")
    ...
    volatility_scalar = target_volatility / max(1e-6, volatility) # Avoid division by zero

    # Limit adjustment range (e.g., 25%-200% of base size)
    volatility_scalar = min(2.0, max(0.25, volatility_scalar))
    logger.info(f"Volatility scalar: {volatility_scalar:.2f} (based on vol={volatility:.4f})")
