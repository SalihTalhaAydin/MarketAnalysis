...
def evaluate_classifier(
    ...
    try:
        ...
        if n_classes == 2:
            ...
            metrics['precision'] = precision_score(y, y_pred, zero_division=0) # Added zero_division
            metrics['recall'] = recall_score(y, y_pred, zero_division=0) # Added zero_division
            ...
            metrics['precision_macro'] = precision_score(y, y_pred, average='macro', zero_division=0) # Added zero_division
            metrics['recall_macro'] = recall_score(y, y_pred, average='macro', zero_division=0) # Added zero_division
            metrics['f1_macro'] = f1_score(y, y_pred, average='macro', zero_division=0) # Added zero_division

            metrics['precision_weighted'] = precision_score(y, y_pred, average='weighted', zero_division=0) # Added zero_division
            metrics['recall_weighted'] = recall_score(y, y_pred, average='weighted', zero_division=0) # Added zero_division
            metrics['f1_weighted'] = f1_score(y, y_pred, average='weighted', zero_division=0) # Added zero_division

            if y_prob is not None:
                # Convert y to one-hot encoding for multiclass ROC AUC
                y_bin = label_binarize(y, classes=unique_classes)

                if n_classes > 2 and y_bin.shape[1] == y_prob.shape[1]: # Check shapes match
    ...
    try:
        if method == 'built_in':
            ...
            if hasattr(model, 'feature_importances_'):
                ...
                logger.warning("Model does not have built-in feature_importances_ or coef_")
        ...
        if method == 'permutation':
            ...
            perm_importance = permutation_importance(
                model, X, y, n_repeats=n_repeats, random_state=42, n_jobs=-1 # Added n_jobs
            )
            ...
            try:
                ...
                if isinstance(shap_values, list): # Multi-class case
                    mean_abs_shap = np.abs(shap_values[1]).mean(axis=0) # Use class 1 for importance
                else: # Binary case
                ...
                return pd.DataFrame({
                 ...
                 logger.warning(f"Could not compute SHAP values: {shap_e}. Falling back to permutation.")
                 return compute_feature_importance(model, X, y, method='permutation', n_repeats=n_repeats)


        else:
            if method == 'shap' and not SHAP_AVAILABLE:
                 logger.warning("SHAP method requested but SHAP library not available. Falling back to permutation.")
                 return compute_feature_importance(model, X, y, method='permutation', n_repeats=n_repeats)
            else:
                 logger.warning(f"Unsupported importance method: {method}. Falling back to permutation.")
                 return compute_feature_importance(model, X, y, method='permutation', n_repeats=n_repeats)
...
def plot_feature_importance(
    ...
    ax = sns.barplot(
        x='Importance',
        y='Feature',
        data=importance_df.sort_values(by='Importance', ascending=False), # Sort for plot
        palette='viridis'
    )
...
def generate_model_report(
    ...
    summary = {
        ...
        'feature_importance_summary': importance_df.head(20).to_dict() if not importance_df.empty else {},
        'model_info': {
    ...
    if output_dir:
        ...
        try:
            with open(report_path, 'w') as f:
                json.dump(summary, f, indent=4, default=str) # Use default=str for non-serializable
