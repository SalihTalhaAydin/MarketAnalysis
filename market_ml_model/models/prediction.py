...
def load_model(model_path: str) -> Tuple[Optional[Any], Dict]:
    ...
    try:
        ...
        if os.path.isdir(model_path):
            ...
            metadata_file = os.path.join(model_path, 'training_summary.json') # Use summary file
            preprocessor_file = os.path.join(model_path, 'preprocessor.pkl')
            features_file = os.path.join(model_path, 'selected_features.json') # Kept for backward compat if summary missing
            ...
            return model, {}
             ...
             logger.error(f"Invalid model path: {model_path}. Must be directory or .pkl file.")
             return None, {}
...
def preprocess_features(
    ...
    try:
        ...
        if selected_features is not None:
            ...
            if missing_features:
                logger.warning(f"Missing expected features in input data: {missing_features}. Proceeding with available features.")
            ...
            logger.info(f"Applied feature selection. Using {len(available_features)} features.")
        else:
        ...
        if preprocessor is not None and SKLEARN_AVAILABLE:
            ...
            try:
                ...
                # Transform the data
                processed_array = preprocessor.transform(features_to_transform) # Pass the correctly column-selected df
                ...
                try:
                    feature_names_out = preprocessor.get_feature_names_out()
                     ...
                     logger.warning("Could not get feature names from preprocessor. Using generic names.")
                     feature_names_out = [f"feat_{i}" for i in range(processed_array.shape[1])]
                ...
                logger.info(f"Preprocessing complete. Output shape: {features_processed.shape}")
                return features_processed

            except ValueError as ve:
                 logger.error(f"ValueError applying preprocessor: {ve}. Check if input features match training features.")
                 return None
            except Exception as e:
    ...
    try:
        ...
        if not hasattr(model, 'predict_proba'):
            ...
            if hasattr(model, 'predict'):
                 logger.warning("Falling back to model.predict() and generating one-hot probabilities.")
                 ...
                 final_class_names = [str(c) for c in unique_classes] if class_names is None else class_names
        ...
        try:
            ...
            if probabilities.shape[1] != len(final_class_names):
                 logger.error(f"Mismatch between probability columns ({probabilities.shape[1]}) and class names ({len(final_class_names)}).")
            ...
            return None, []
             ...
             logger.error(f"ValueError during predict_proba: {ve}. Check feature consistency.")
             return None, []
        except Exception as e:
...
def predict_with_threshold(
    ...
    if n_classes == 2:
        ...
        if positive_class_index >= n_classes:
             logger.error(f"positive_class_index ({positive_class_index}) out of bounds for {n_classes} classes.")
        ...
        return np.where(probabilities[:, positive_class_index] >= threshold, positive_class_index, 1 - positive_class_index)
    elif n_classes > 2:
        ...
        return np.argmax(probabilities, axis=1)
         ...
         logger.warning("Probabilities array has only one column. Applying threshold >= 0.5.")
...
class ModelPredictorBase:
    ...
    def _create_explainer(self):
        ...
        try:
            ...
            if 'xgb' in model_type_name or 'lgbm' in model_type_name or 'catboost' in model_type_name:
                 ...
                 logger.warning("Creating SHAP KernelExplainer (may be slow). Background data needed for optimal use.")
                 # self.explainer = shap.KernelExplainer(self.model.predict_proba, background_data) # Needs background data
                 self.explainer = None # Disable KernelExplainer by default due to complexity
            else:
                 logger.warning(f"SHAP explainer not supported for model type {type(self.model).__name__}")
    ...
    def predict(self, features: pd.DataFrame) -> Optional[np.ndarray]:
        ...
        if self.processed_feature_names and list(features_processed.columns) != self.processed_feature_names:
             logger.warning("Input columns after preprocessing do not match expected processed feature names. Reordering/selecting.")
             try:
                 features_processed = features_processed[self.processed_feature_names]
             except KeyError as e:
                 logger.error(f"Missing expected processed features after preprocessing: {e}")
                 return None
        ...
        try:
            ...
            return predictions
             ...
             logger.error(f"ValueError during prediction: {ve}. Check feature consistency.")
             return None
        except Exception as e:
            logger.exception(f"Error generating predictions: {e}")
            return None

    def predict_proba(self, features: pd.DataFrame) -> Tuple[Optional[np.ndarray], List[str]]:
        """
    ...
    def explain(
        ...
        try:
            ...
            # Ensure columns match processed feature names if available
            if self.processed_feature_names and list(features_processed.columns) != self.processed_feature_names:
                 logger.warning("Input columns for explanation do not match expected processed feature names. Reordering/selecting.")
                 try:
                     features_processed = features_processed[self.processed_feature_names]
                 except KeyError as e:
                     logger.error(f"Missing expected processed features for explanation: {e}")
                     return None
            ...
            if max_samples is not None and len(features_processed) > max_samples:
                ...
                features_sample = features_processed.sample(max_samples, random_state=42)
            else:
                features_sample = features_processed

            if features_sample.empty:
                 logger.warning("No samples available for SHAP explanation after preprocessing/sampling.")
                 return None

            # Generate SHAP values
            logger.info(f"Calculating SHAP values for {len(features_sample)} samples...")
            ...
            elif isinstance(shap_values, np.ndarray): # Binary case (TreeExplainer) or KernelExplainer
                shap_values_list = shap_values.tolist()
                base_values = self.explainer.expected_value
            else: # Newer SHAP object structure
                 shap_values_list = shap_values.values.tolist() if hasattr(shap_values, 'values') else None
            ...
            shap_dict = {
                ...
                'feature_names': list(features_sample.columns), # Use columns from processed sample
    ...
    def plot_shap_summary(
        ...
        if self.explainer is None or not SHAP_AVAILABLE or not VISUALIZATION_AVAILABLE:
            logger.warning("SHAP explainer or visualization libraries not available, cannot plot summary.")
            return
        ...
        try:
            ...
            if features_processed is None: return

            # Ensure columns match processed feature names if available
            if self.processed_feature_names and list(features_processed.columns) != self.processed_feature_names:
                 logger.warning("Input columns for SHAP plot do not match expected processed feature names. Reordering/selecting.")
                 try:
                     features_processed = features_processed[self.processed_feature_names]
                 except KeyError as e:
                     logger.error(f"Missing expected processed features for SHAP plot: {e}")
                     return


            # Limit samples if needed
            if max_samples is not None and len(features_processed) > max_samples:
                logger.info(f"Limiting SHAP calculation for plot to {max_samples} samples")
                features_sample = features_processed.sample(max_samples, random_state=42)
            else:
                features_sample = features_processed

            if features_sample.empty:
                 logger.warning("No samples available for SHAP plot after preprocessing/sampling.")
                 return

            # Generate SHAP values
            logger.info(f"Calculating SHAP values for plot ({len(features_sample)} samples)...")
            start_time = time.time()
            ...
            try:
                 ...
                 if isinstance(shap_values_for_plot, list):
                      shap_values_for_plot = shap_values_for_plot[1] # Default to class 1 for summary plot
            ...
            shap.summary_plot(
                shap_values_for_plot, features_sample,
...
class PredictionManager:
    ...
    def add_predictor(
        ...
        try:
            ...
            elif predictor.class_names is not None and self.class_names != predictor.class_names:
                 logger.error(f"Class name mismatch for predictor '{name}'. Expected {self.class_names}, got {predictor.class_names}.")
                 return False


            self.predictors[name] = predictor
            self.ensemble_weights[name] = weight
            logger.info(f"Added predictor '{name}' from {model_path} with weight {weight}.")
            return True
    ...
    def predict_proba(
        ...
        for name, predictor in self.predictors.items():
            ...
            if probs is None:
                logger.warning(f"Predictor '{name}' failed to generate probabilities. Skipping.")
                continue
            # Ensure class name consistency was checked during add_predictor
            if not final_class_names: final_class_names = names # Set if first predictor
        ...
        if ensemble_method == 'average':
            ...
            total_weight = sum(self.ensemble_weights[name] for name, predictor in self.predictors.items() if name in self.ensemble_weights) # Recalculate in case some failed
            if total_weight == 0:
    ...
    def predict_with_confidence(
        ...
        predictions = predict_with_threshold(ensembled_probs) # Use default threshold for prediction
...
def validate_model_predictions(
    ...
    try:
        ...
        from market_ml_model.models.evaluation.metrics import evaluate_classifier # Avoid circular import if possible
        # Or recalculate specific metrics here
        full_metrics = evaluate_classifier(model, X_val_processed, y_val) # Use existing evaluator

        for metric in metrics_to_check:
             if metric in full_metrics:
                 validation_results[metric] = full_metrics[metric]
             elif metric == 'roc_auc' and 'roc_auc_ovr' in full_metrics: # Handle multiclass AUC
...
class SignalGenerator:
    ...
    def __init__(
        ...
        if self.class_names:
             try:
                 ...
                 self.neg_idx = self.class_names.index('-1') if '-1' in self.class_names else (self.class_names.index('0') if '0' in self.class_names and len(self.class_names)==2 else 0)
                 self.pos_idx = self.class_names.index('1') if '1' in self.class_names else 1
                 logger.info(f"Signal mapping: Negative Class Index={self.neg_idx}, Positive Class Index={self.pos_idx}")
             except ValueError:
                 logger.warning(f"Could not automatically map class names {self.class_names} to -1/1 signals. Assuming binary [0, 1] or multiclass [0, 1, ...].")
                 self.neg_idx = 0
                 self.pos_idx = 1 # Default assumption
        else:
             logger.warning("Class names not found in predictor metadata. Assuming binary [0, 1] output.")
             self.neg_idx = 0
             self.pos_idx = 1
    ...
    def generate_signals(
        ...
        """
        if features.empty:
             return pd.DataFrame(columns=['probability_pos', 'probability_neg', 'signal', 'confidence'])
        ...
        if probabilities.shape[1] < 2:
             logger.error(f"Probabilities have unexpected shape: {probabilities.shape}. Need at least 2 classes.")
             return pd.DataFrame(columns=['probability_pos', 'probability_neg', 'signal', 'confidence'])

        # Ensure class names match if provided earlier
        if self.class_names and self.class_names != class_names:
             logger.warning(f"Class names mismatch during prediction. Expected {self.class_names}, got {class_names}. Re-evaluating indices.")
             # Re-attempt index finding
             try:
                 self.neg_idx = class_names.index('-1') if '-1' in class_names else (class_names.index('0') if '0' in class_names and len(class_names)==2 else 0)
                 self.pos_idx = class_names.index('1') if '1' in class_names else 1
                 logger.info(f"Updated Signal mapping: Neg Idx={self.neg_idx}, Pos Idx={self.pos_idx}")
             except ValueError:
                 logger.error(f"Could not map new class names {class_names}. Cannot generate signals.")
                 return pd.DataFrame(columns=['probability_pos', 'probability_neg', 'signal', 'confidence'])


        prob_pos = probabilities[:, self.pos_idx]
        # Handle case where negative class might not exist explicitly (e.g., binary [0, 1])
        prob_neg = probabilities[:, self.neg_idx] if self.neg_idx < probabilities.shape[1] else (1 - prob_pos if probabilities.shape[1]==2 else 0)
        ...
        if self.signal_type == 'threshold':
            ...
            signals.loc[prob_neg >= self.threshold, 'raw_signal'] = -1 # Assumes neg prob available
            # Refine for binary case where only pos prob matters vs threshold
            if probabilities.shape[1] == 2:
                 signals['raw_signal'] = 0
                 signals.loc[prob_pos >= self.threshold, 'raw_signal'] = 1
                 signals.loc[prob_pos <= (1 - self.threshold), 'raw_signal'] = -1 # Signal short if prob_pos is low
             ...
             logger.warning(f"Unsupported signal_type: {self.signal_type}. Using default threshold.")
             signals.loc[prob_pos >= self.threshold, 'raw_signal'] = 1
        ...
        if ohlc_data is not None and not ohlc_data.empty:
             ...
             if self.trend_filter_ma is not None:
                 signals['filtered_signal'] = self._apply_trend_filter(signals, ohlc_data_aligned)

             # Apply Volatility Filter
             if self.volatility_filter_atr is not None:
                 signals['filtered_signal'] = self._apply_volatility_filter(signals, ohlc_data_aligned)
        else:
             if self.trend_filter_ma is not None or self.volatility_filter_atr is not None:
                 logger.warning("OHLC data not provided, cannot apply trend/volatility filters.")
    ...
    def _apply_trend_filter(self, signals: pd.DataFrame, ohlc: pd.DataFrame) -> pd.Series:
    ...
    def _apply_volatility_filter(self, signals: pd.DataFrame, ohlc: pd.DataFrame) -> pd.Series:
        ...
        if not all(c in ohlc.columns for c in req_cols):
             logger.warning(f"Volatility filter requires {req_cols} columns in OHLC data.")
             return signals['filtered_signal']

        try:
            if not SKLEARN_AVAILABLE: # Need ATR calculation fallback if TA lib missing
                 logger.warning("Cannot calculate ATR for volatility filter without TA-Lib/pandas-ta.")
                 return signals['filtered_signal']

            # Calculate ATR (using pandas_ta if available, basic otherwise)
            try:
                 import pandas_ta as ta
                 atr = ta.atr(ohlc['high'], ohlc['low'], ohlc['close'], length=self.volatility_filter_atr)
            except ImportError:
            ...
            if atr is None or atr.isna().all():
                 logger.warning("ATR calculation failed, cannot apply volatility filter.")
                 return signals['filtered_signal']
            ...
            logger.info(f"Applied volatility filter (ATR {self.volatility_filter_atr} > {self.volatility_threshold_q:.0%} quantile).")
            return filtered
        except Exception as e:
        ...
        for i in range(len(signals)):
            if signals['signal'].iloc[i] != 0:
                if i - last_signal_idx > self.cooling_period:
                    ...
                    signals['signal'].iloc[i] = 0 # Suppress signal due to cooling period
    ...
    def plot_signal_history(
        ...
        try:
            ...
            # Plot Price
            axes[0].plot(ohlc_aligned.index, ohlc_aligned['close'], label='Close Price', color='blue', alpha=0.8)
            ...
            axes[0].scatter(long_signals.index, ohlc_aligned.loc[long_signals.index, 'close'] * 0.99,
                            label='Long Signal', marker='^', color='green', s=100, alpha=0.9, zorder=5)
            axes[0].scatter(short_signals.index, ohlc_aligned.loc[short_signals.index, 'close'] * 1.01,
                            label='Short Signal', marker='v', color='red', s=100, alpha=0.9, zorder=5)
            axes[0].legend()


            # Plot Probabilities
            axes[1].plot(signals_aligned.index, signals_aligned['probability_pos'], label='Prob(Long)', color='green', alpha=0.7)
            axes[1].plot(signals_aligned.index, signals_aligned['probability_neg'], label='Prob(Short)', color='red', alpha=0.7)
            axes[1].axhline(0.5, color='grey', linestyle='--', alpha=0.5)
            if self.signal_type == 'threshold':
                 axes[1].axhline(self.threshold, color='black', linestyle=':', alpha=0.6, label=f'Threshold ({self.threshold:.2f})')
                 axes[1].axhline(1-self.threshold, color='black', linestyle=':', alpha=0.6)
            elif self.signal_type == 'probability_weighted':
                 axes[1].axhline(self.neutral_zone[0], color='orange', linestyle=':', alpha=0.6, label=f'Neutral Zone ({self.neutral_zone[0]:.2f}-{self.neutral_zone[1]:.2f})')
...
class PredictionScheduler:
    ...
    def predict_once(
        self,
        ticker: str,
        start_date: Optional[str] = None, # For fetching historical data needed for features
        ...
        try:
            ...
            if end_date is None: end_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            if start_date is None:
                 ...
                 logger.warning("Start date not provided, fetching recent data. Feature accuracy may be affected.")
                 # Use data_loader's defaults if possible
                 start_date = self.data_loader.config.default_start_date


            raw_data = self.data_loader.load_data(ticker=ticker, start_date=start_date, end_date=end_date)
            ...
            result_df = pd.DataFrame(probabilities, index=latest_features.index, columns=class_names)
            result_df['prediction'] = predict_with_threshold(probabilities) # Use default threshold
            result_df['confidence'] = get_confidence_levels(probabilities)

            logger.info(f"Prediction for {ticker} at {result_df.index[0]}: {result_df.iloc[0].to_dict()}")
    ...
    def predict_batch(self, tickers: List[str], **kwargs) -> Dict[str, Optional[pd.DataFrame]]:
    ...
    def start_scheduler(self):
        ...
        if not self.scheduler_active:
             logger.info(f"Starting prediction scheduler with interval {self.interval}...")
    ...
    def _run_scheduler(self):
        ...
        while self.scheduler_active:
            now = datetime.now()
            if self.last_prediction_time is None or (now - self.last_prediction_time >= self.interval):
            ...
            for _ in range(int(sleep_seconds / check_interval)):
                 if not self.scheduler_active: break # Exit loop if stopped
                 time.sleep(check_interval)
            if not self.scheduler_active: break # Exit while loop if stopped


    def _save_prediction_results(self, ticker: str, results: pd.DataFrame):
        """Save prediction results to a file."""
        if not self.output_dir or results.empty: return
        try:
