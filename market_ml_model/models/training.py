...
def create_feature_pipeline(
        ...
        for col in X.columns:
            if X[col].dtype == 'object' or pd.api.types.is_categorical_dtype(X[col]):
                ...
                numeric_features.append(col)
                 ...
                 logger.warning(f"Column '{col}' has unhandled dtype {X[col].dtype}, skipping.")
    ...
    if unclassified_in_df:
        logger.warning(f"Features in DataFrame but not classified: {unclassified_in_df}. Attempting auto-classification.")
    ...
    if numeric_features:
        ...
        if scaling_method == 'standard':
            ...
            numeric_transformer_steps.append(
             ...
             logger.warning(f"Unsupported scaling method: {scaling_method}. No scaling applied.")
    ...
    if categorical_features:
        ...
        onehot_encoder = OneHotEncoder(handle_unknown='ignore', sparse_output=False) # Use sparse_output=False for easier name handling
...
def train_classification_model(
    ...
    fs_params = feature_selection_config.get('params', {'n_features': min(50, features.shape[1])})
    ...
    opt_params = optimization_config.get('params', {}) # Params for the method (e.g., n_iter)
    ...
    try:
        if isinstance(features.index, pd.DatetimeIndex):
            ...
            X_train_raw, X_test_raw = features.iloc[:split_idx], features.iloc[split_idx:]
            y_train, y_test = target.iloc[:split_idx], target.iloc[split_idx:]
            logger.info(f"Split data using time series method: {len(X_train_raw)} train, {len(X_test_raw)} test")
        else:
            # Regular split
            X_train_raw, X_test_raw, y_train, y_test = train_test_split(
                features, target, test_size=test_size, random_state=random_state, stratify=target # Added stratify
            )
            logger.info(f"Split data using random stratified split: {len(X_train_raw)} train, {len(X_test_raw)} test")
    ...
    # --- Preprocessing ---
    try:
        logger.info(f"Creating preprocessing pipeline (Scaling: {scaling_method}, Missing: {handle_missing})")
        ...
        current_num_features = [f for f in selected_features if f in X_train_fs.columns and pd.api.types.is_numeric_dtype(X_train_fs[f])] if num_features is None else [f for f in num_features if f in selected_features]
        current_cat_features = [f for f in selected_features if f not in current_num_features] if cat_features is None else [f for f in cat_features if f in selected_features]
        ...
        try:
             ...
             logger.warning("Could not get feature names from preprocessor. Using original selected names.")
             feature_names_out = selected_features # Fallback

        # Convert processed data back to DataFrame (optional, but good for inspection)
        X_train_processed = pd.DataFrame(X_train_processed, index=X_train_fs.index, columns=feature_names_out)
        X_test_processed = pd.DataFrame(X_test_processed, index=X_test_fs.index, columns=feature_names_out)
    ...
    try:
        if optimize_hyperparams:
            logger.info(f"Optimizing hyperparameters for {model_type} using {opt_method} method")
            ...
            logger.info(f"Hyperparameter optimization complete. Best parameters found: {best_params}")
        ...
        logger.info(f"Test Set Metrics: Accuracy={test_metrics.get('accuracy', 'N/A'):.4f}, F1-Weighted={test_metrics.get('f1_weighted', 'N/A'):.4f}")
        ...
        logger.info(f"Feature importance calculated. Top 5: {importance_df['Feature'].head().tolist()}")
        ...
        if model_dir:
            ...
            final_config = {
                ...
                'optimization_config': optimization_config if optimize_hyperparams else None,
            ...
            try:
                with open(config_path, 'w') as f:
                    json.dump(final_config, f, indent=4, default=str) # Use default=str for non-serializable
        ...
        logger.exception(f"Critical error during model training pipeline: {e}") # Log traceback
