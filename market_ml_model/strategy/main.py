...
try:
    ...
    from ..models.training import train_classification_model, create_feature_pipeline # Added pipeline creation
    from ..models.prediction import ModelPredictorBase, PredictionManager, SignalGenerator, predict_with_model, load_model # Added load_model
    from ..models.evaluation.metrics import generate_model_report # Added report generation
    ...
    from ..utils.visualization import plot_equity_curve, plot_monthly_returns, plot_drawdowns # Keep for potential use
    ...
    try:
        ...
        class KMeans: pass # Dummy class
        class StandardScaler: pass # Dummy class

    MODULES_AVAILABLE = True
except ImportError as e:
     # Fallback for running script directly (less ideal)
     logger.error(f"Error importing strategy components: {e}. Relative imports failed. Ensure running as part of the package.")
     ...
     def train_classification_model(*args, **kwargs): return None, None, None
     class ModelPredictorBase: pass
     class PredictionManager: pass
     class SignalGenerator: pass
     def predict_with_model(*args, **kwargs): return None
     def load_model(*args, **kwargs): return None, {}
     def generate_model_report(*args, **kwargs): return {}
     def backtest_strategy(*args, **kwargs): return {}
     class TradeManager: pass
     def calculate_returns_metrics(*args, **kwargs): return {}
     def plot_equity_curve(*args, **kwargs): pass
     def plot_monthly_returns(*args, **kwargs): pass
     def plot_drawdowns(*args, **kwargs): pass
     class KMeans: pass
...
class ModelConfig:
    ...
    def __init__(
        ...
        ensemble_models: Optional[List[str]] = None, # List of models for ensemble, None for single model
        ...
        early_stopping_rounds: Optional[int] = 50, # For models that support it (e.g., XGB, LGBM)
        # Prediction / Signal Generation
        probability_threshold: float = 0.60, # Threshold for converting prob to signal
        signal_neutral_zone: Tuple[float, float] = (0.45, 0.55), # Prob range for neutral signal
        ...
        regime_models: Optional[Dict[int, str]] = None # Optional: map regime -> model_type
    ):
...
class WalkForwardConfig:
    ...
    def __init__(
        ...
        early_stopping_drawdown: Optional[float] = 0.25, # Stop WF if portfolio DD exceeds this
...
class StrategyConfig:
    ...
    def from_dict(cls, config_dict: Dict) -> 'StrategyConfig':
        ...
        feature_config = FeatureConfig.from_dict(feature_config_dict) if feature_config_dict else FeatureConfig()

        model_config_dict = config_dict.pop('model_config', {})
        model_config = ModelConfig.from_dict(model_config_dict) if model_config_dict else ModelConfig()

        walkforward_config_dict = config_dict.pop('walkforward_config', {})
        walkforward_config = WalkForwardConfig.from_dict(walkforward_config_dict) if walkforward_config_dict else WalkForwardConfig()
...
class MarketRegimeDetector:
    ...
    def __init__(
        ...
        self.regime_features = regime_features or [
            'volatility_ratio_20_50', 'efficiency_ratio_20', 'adf_pvalue' # Example features
        ]
        self.use_clustering = use_clustering
        self.scaler = StandardScaler() if SKLEARN_AVAILABLE_FOR_REGIME else None
        self.kmeans = KMeans(n_clusters=n_regimes, random_state=42, n_init=10) if SKLEARN_AVAILABLE_FOR_REGIME else None
    ...
    def detect_regime(self, data: pd.DataFrame) -> Optional[Dict]:
        ...
        if missing_features:
            logger.error(f"Missing required features for regime detection: {missing_features}")
            return None
        ...
        if len(data) < self.lookback_window:
             logger.warning(f"Not enough data ({len(data)}) for regime lookback window ({self.lookback_window}).")
        ...
        if len(regime_data) < self.lookback_window:
             logger.warning(f"Not enough non-NaN data ({len(regime_data)}) in lookback window for regime detection.")
             return None
        ...
        if self.use_clustering:
            ...
            logger.warning("Rule-based regime detection not implemented yet. Using clustering.")
            regime_info = self._detect_clustering_regime(regime_data)
    ...
    def _detect_clustering_regime(self, regime_data: pd.DataFrame) -> Optional[Dict]:
        ...
        if not SKLEARN_AVAILABLE_FOR_REGIME or self.scaler is None or self.kmeans is None:
        ...
        try:
            ...
            if not self.is_fitted:
                 ...
                 self.is_fitted = True
                 logger.info(f"KMeans model fitted. Cluster centers:\n{self.kmeans.cluster_centers_}")
            ...
            regime_names = {0: "Regime A", 1: "Regime B", 2: "Regime C"} # Needs proper mapping
            ...
            regime_info = {
                ...
                'features': regime_data.iloc[-1].to_dict(), # Features for the detected point
    ...
    def plot_regime_history(
        ...
        try:
            ...
            if 'timestamp' not in regime_df.columns or regime_df['timestamp'].isnull().all():
            ...
            if price_data is not None and not price_data.empty:
                ...
                ax1.plot(price_data_aligned.index, price_data_aligned, label='Price', color='black', alpha=0.6, linewidth=1)
            ...
            ax2.plot(regime_df.index, regime_df['regime'], drawstyle='steps-post',
                     label='Detected Regime', color='blue', linewidth=1.5)
...
class EnhancedTradingStrategy:
    ...
    def __init__(self, config: StrategyConfig):
        ...
        if not MODULES_AVAILABLE:
             raise ImportError("Core strategy modules could not be imported. Cannot initialize strategy.")

        self.config = config
        self.data_loader = self._initialize_data_loader()
        self.regime_detector = MarketRegimeDetector(n_regimes=3) if config.model_config.regime_adaptation_enabled else None # Example: 3 regimes
        self.models: Dict[str, str] = {} # Stores paths to saved models (asset_symbol[_regime_X] -> path)
        self.predictors: Dict[str, ModelPredictorBase] = {} # Stores loaded predictors
        self.signal_generators: Dict[str, SignalGenerator] = {} # Stores signal generators per asset/model_key
        self.results: Dict[str, Any] = {} # Stores results per asset or walk-forward step
    ...
    def _initialize_data_loader(self) -> Optional[DataLoader]:
        ...
        try:
            ...
            loader_config = DataLoaderConfig(cache_dir=cache_dir, use_cache=True) # Enable caching
    ...
    def load_data(self, asset_config: AssetConfig) -> Optional[pd.DataFrame]:
        ...
        logger.info(f"Loading data for {asset_config.symbol} ({asset_config.timeframe}) from {self.config.data_start_date} to {self.config.data_end_date}...")
        try:
            ...
            logger.debug(f"Standardized columns: {original_cols} -> {list(raw_data.columns)}")
            ...
            if not isinstance(raw_data.index, pd.DatetimeIndex):
                 try:
                     raw_data.index = pd.to_datetime(raw_data.index)
                      ...
                      logger.error(f"Failed to convert index to DatetimeIndex for {asset_config.symbol}: {e}")
            ...
            if initial_nans > 0:
                 ...
                 logger.debug(f"Forward-filled {initial_nans} initial NaNs for {asset_config.symbol}.")

            # Drop any remaining NaNs at the beginning
            raw_data = raw_data.dropna()

            if raw_data.empty:
                 logger.error(f"Data empty after initial preprocessing for {asset_config.symbol}")
                 return None

            logger.info(f"Data loaded and preprocessed for {asset_config.symbol}. Shape: {raw_data.shape}")
            return raw_data
    ...
    def engineer_features(
        ...
        try:
            ...
            if target_col and target_col in feature_df.columns:
                 ...
                 target = target.loc[features.index] # Align target with dropped features
                 if len(features) < original_len:
                      logger.warning(f"Dropped {original_len - len(features)} rows due to NaNs in engineered features.")

                 if features.empty:
                      logger.error("Features DataFrame empty after dropping NaNs.")
                      return None, None

                 logger.info(f"Features engineered. Shape: {features.shape}. Target: '{target_col}' ({len(target)} samples)")
                 return features, target
            else:
                 logger.error(f"Target column '{target_col}' not found after feature engineering.")
                 # Check available columns for debugging
                 logger.debug(f"Available columns after feature engineering: {list(feature_df.columns)}")
                 return None, None

        except Exception as e:
            logger.exception(f"Error engineering features for {asset_config.symbol}: {e}")
            return None, None
    ...
    def train_model(
        ...
        if regime is not None and self.config.model_config.regime_adaptation_enabled:
             ...
             logger.info(f"Training model for {asset_config.symbol} in Regime {regime}...")
        else:
        ...
        if fold_id:
             ...
             model_output_dir = os.path.join(model_base_dir, f"run_{datetime.now().strftime('%Y%m%d%H%M%S')}")
        ...
        try:
            ...
            train_opt_config = {
                 ...
                 'params': {'n_iter': self.config.model_config.optimization_trials}, # Pass n_iter/n_trials
            ...
            logger.info(f"Model training successful for {model_key}. Saved to: {model_output_dir}")
    ...
    def generate_predictions(
        ...
        if regime is not None and self.config.model_config.regime_adaptation_enabled:
             ...
             potential_regime_path = os.path.join(self.config.output_dir, "models", asset_config.symbol, f"regime_{regime}")
             if fold_id: potential_regime_path = os.path.join(potential_regime_path, fold_id)
             ...
             if regime_key in self.models and os.path.exists(self.models[regime_key]):
                  ...
                  logger.warning(f"No specific model found for regime {regime}. Using base model for {asset_config.symbol}.")
        ...
        if fold_id:
             ...
             try:
                  latest_run = sorted([d for d in os.listdir(model_base_dir) if d.startswith('run_')], reverse=True)[0]
                  model_path = os.path.join(model_base_dir, latest_run)
             except (FileNotFoundError, IndexError):
                  logger.error(f"No trained model runs found in {model_base_dir} for key '{model_key}'.")
                  return None


        logger.info(f"Generating predictions for {asset_config.symbol} using model from '{model_path}'...")
        ...
        try:
            ...
            result_df = pd.DataFrame(probabilities, index=features.index, columns=class_names)
            ...
            try:
                 ...
                 logger.warning(f"Positive class label '{positive_class_label}' not found in {class_names}. Defaulting to index 1.")
            ...
            logger.info(f"Predictions generated successfully for {asset_config.symbol}.")
            return result_df

        except Exception as e:
            logger.exception(f"Error generating predictions using model from {model_path}: {e}")
            return None
    ...
    def generate_signals(
        ...
        if regime is not None and self.config.model_config.regime_adaptation_enabled:
             ...
             predictor_key_base = os.path.join(self.config.output_dir, "models", asset_config.symbol) # Path logic needs refinement
             predictor_key_regime = os.path.join(self.config.output_dir, "models", asset_config.symbol, f"regime_{regime}")
             ...
             predictor = self.predictors.get(regime_key) or self.predictors.get(model_key)
        ...
        if signal_gen_key not in self.signal_generators:
             try:
                  ...
                  logger.error(f"Failed to initialize SignalGenerator for '{signal_gen_key}': {e}")
                  return None
        ...
        try:
            ...
            if signals_df is None:
                 logger.error(f"Signal generation function returned None for {asset_config.symbol}.")
                 return None
    ...
    def backtest_signals(
        ...
        backtest_data['prediction'] = signals_aligned['signal'] # Use 'signal' as prediction input

        # --- Determine ATR column name ---
        # Try to find a suitable ATR column (e.g., ATRr_10, atr_14)
        atr_col_name = None
        possible_atr_cols = [f'atr_{self.config.feature_config.max_holding_period}', 'ATRr_10', 'atr_14']
        ...
        if atr_col_name is None:
             logger.warning(f"Could not find a suitable ATR column in {list(backtest_data.columns)}. Dynamic stops might use fixed percentages.")
        ...
        try:
            ...
            backtest_output_base = os.path.join(self.config.output_dir, "backtests", asset_config.symbol)
            if fold_id:
                 backtest_output_dir = os.path.join(backtest_output_base, fold_id)
            else:
                 backtest_output_dir = os.path.join(backtest_output_base, f"run_{datetime.now().strftime('%Y%m%d%H%M%S')}")
    ...
    def run_walk_forward_validation(self, asset_config: AssetConfig) -> Dict:
        ...
        logger.info(f"--- Starting Walk-Forward Validation for {asset_config.symbol} ---")
        ...
        if full_data is None or len(full_data) < wf_config.initial_train_periods + wf_config.test_periods:
            logger.error(f"Not enough data for walk-forward validation ({len(full_data)} periods). Need at least {wf_config.initial_train_periods + wf_config.test_periods}.")
        ...
        while test_end_idx <= n:
            ...
            if step == 1 or (step - 1) % wf_config.retrain_frequency == 0:
                ...
                train_features, train_target = self.engineer_features(train_data, asset_config)
                if train_features is None:
                     logger.error(f"Feature engineering failed for training step {step}. Skipping step.")
                     # Advance window indices based on config
                     train_start_idx = train_start_idx + wf_config.step_periods if wf_config.rolling_window else train_start_idx
                     train_end_idx += wf_config.step_periods
                     test_end_idx += wf_config.step_periods
                     continue

                # 2. Train model (saves model to fold-specific dir)
                model_path = self.train_model(train_features, train_target, asset_config, fold_id=fold_id)
                if model_path is None:
                     logger.error(f"Model training failed for step {step}. Skipping step.")
                 ...
                 last_retrain_step = ((step - 2) // wf_config.retrain_frequency) * wf_config.retrain_frequency + 1
                 ...
                 if not os.path.exists(os.path.join(model_path, 'model.pkl')): # Check if model file exists
                      logger.error(f"Model file not found at expected path {model_path} for step {step}. Retraining required?")
            ...
            test_features, _ = self.engineer_features(test_data, asset_config) # Target not needed
            if test_features is None:
                 logger.error(f"Feature engineering failed for test step {step}. Skipping step.")
                 # Advance window indices
                 train_start_idx = train_start_idx + wf_config.step_periods if wf_config.rolling_window else train_start_idx
                 train_end_idx += wf_config.step_periods
                 test_end_idx += wf_config.step_periods
            ...
            predictions_df = self.generate_predictions(test_features, asset_config, fold_id=fold_id) # Pass fold_id to load correct model

            if predictions_df is None:
                 logger.error(f"Prediction generation failed for step {step}. Skipping step.")
                 # Advance window indices
                 train_start_idx = train_start_idx + wf_config.step_periods if wf_config.rolling_window else train_start_idx
                 train_end_idx += wf_config.step_periods
                 test_end_idx += wf_config.step_periods
            ...
            # --- Generate Signals ---
            signals_df = self.generate_signals(predictions_df, test_data, asset_config) # Pass OHLC test data
            if signals_df is None:
                 logger.error(f"Signal generation failed for step {step}. Skipping step.")
                 # Advance window indices
                 train_start_idx = train_start_idx + wf_config.step_periods if wf_config.rolling_window else train_start_idx
                 train_end_idx += wf_config.step_periods
                 test_end_idx += wf_config.step_periods
            ...
            if step_results:
                 ...
                 step_results['train_start'] = train_data.index[0].isoformat() if isinstance(train_data.index, pd.DatetimeIndex) else train_start_idx
                 step_results['train_end'] = train_data.index[-1].isoformat() if isinstance(train_data.index, pd.DatetimeIndex) else train_end_idx - 1
                 step_results['test_start'] = test_data.index[0].isoformat() if isinstance(test_data.index, pd.DatetimeIndex) else train_end_idx
                 step_results['test_end'] = test_data.index[-1].isoformat() if isinstance(test_data.index, pd.DatetimeIndex) else test_end_idx - 1
                 wf_results_list.append(step_results)
                 logger.info(f"Step {step} Backtest Summary: Return={step_results.get('total_return_pct', 0):.2f}%, Trades={step_results.get('num_trades', 0)}")

                 # --- Collect trades from step ---
                 # Read trades file saved by backtest_strategy
                 step_trades_path = os.path.join(self.config.output_dir, "backtests", asset_config.symbol, fold_id, "trades.csv")
                 if os.path.exists(step_trades_path):
                      try:
                           ...
                           logger.warning(f"Could not read trades file for step {step}: {e}")
            ...
            if not wf_config.rolling_window and train_end_idx - train_start_idx < wf_config.min_train_periods:
                 logger.warning(f"Training window below minimum size ({wf_config.min_train_periods}). Stopping walk-forward.")
                 break


        logger.info(f"--- Walk-Forward Validation Completed for {asset_config.symbol} ---")

        # Combine results
        wf_summary = self.summarize_walk_forward_results(wf_results_list, asset_config) # Pass list of dicts

        # Combine all trades if collected
        combined_trades_df = pd.concat(all_trades_list, ignore_index=True) if all_trades_list else pd.DataFrame()
        if not combined_trades_df.empty:
             trades_summary_path = os.path.join(self.config.output_dir, "walk_forward", asset_config.symbol, "all_wf_trades.csv")
             try:
                  os.makedirs(os.path.dirname(trades_summary_path), exist_ok=True)
                  combined_trades_df.to_csv(trades_summary_path, index=False)
                  logger.info(f"Combined walk-forward trades saved to {trades_summary_path}")
             except Exception as e:
                  logger.error(f"Failed to save combined WF trades: {e}")


        # Plot overall WF performance
        wf_plot_path = os.path.join(self.config.output_dir, "plots", f"{asset_config.symbol}_walk_forward_performance.png")
        self.plot_walk_forward_results(wf_results_list, combined_trades_df, asset_config, full_data, filename=wf_plot_path)


        return wf_summary


    def summarize_walk_forward_results(self, wf_results_list: List[Dict], asset_config: AssetConfig) -> Dict:
        ...
        overall_metrics["profitable_folds_pct"] = (overall_metrics["profitable_folds"] / overall_metrics["num_folds"]) * 100 if overall_metrics["num_folds"] > 0 else 0
        ...
        try:
            ...
            if years > 0:
                overall_metrics['annualized_return_pct'] = ((1 + compounded_return) ** (1 / years) - 1) * 100
        ...
        summary_path = os.path.join(self.config.output_dir, "walk_forward", asset_config.symbol, "walk_forward_summary.csv")
        try:
        ...
        summary_dict_path = os.path.join(self.config.output_dir, "walk_forward", asset_config.symbol, "walk_forward_overall_summary.json")
        try:
    ...
    def plot_walk_forward_results(
        ...
        try:
            ...
            if not combined_trades.empty and 'exit_time' in combined_trades.columns:
                 ...
                 trades_sorted['pnl_abs'] = (trades_sorted['exit_price'] - trades_sorted['entry_price']) * trades_sorted['size'] * trades_sorted['direction']
                 # Need initial capital to build equity curve accurately
                 initial_cap = 100000 # Assume same as backtest start
                 trades_sorted['equity'] = initial_cap + trades_sorted['pnl_abs'].cumsum()
                 ...
                 full_equity = equity_curve.reindex(full_data.index).ffill().fillna(initial_cap)

                 plot_equity_curve(full_equity, title=f"{asset_config.symbol} Walk-Forward Equity Curve", ax=axes[0]) # Pass ax
            else:
                 axes[0].set_title(f"{asset_config.symbol} Walk-Forward Equity (Trades Missing)")
            ...
            rolling_sharpe = summary_df['sharpe_ratio'].rolling(self.config.walkforward_config.performance_tracking_window).mean()
            axes[2].plot(summary_df['fold_number'], summary_df['sharpe_ratio'], label='Fold Sharpe', marker='.', linestyle=':', alpha=0.6)
            axes[2].plot(summary_df['fold_number'], rolling_sharpe, label=f'Rolling Sharpe ({self.config.walkforward_config.performance_tracking_window} folds)', color='orange')
            ...
            logger.exception(f"Error plotting walk-forward results for {asset_config.symbol}: {e}")
            plt.close() # Ensure plot closed on error


    def run_strategy(self) -> Dict:
        """Run the full strategy workflow for all configured assets."""
        logger.info(f"--- Running Strategy: {self.config.strategy_name} (ID: {self.config.strategy_run_id}) ---")
        ...
        Executor = concurrent.futures.ThreadPoolExecutor if use_parallel else concurrent.futures.SequentialExecutor # Use SequentialExecutor for no parallelism

        logger.info(f"Parallel Processing: {'Enabled' if use_parallel else 'Disabled'} (Max Workers: {max_workers})")
        ...
        with Executor(max_workers=max_workers) as executor:
            ...
            for asset_config in self.config.assets:
                 ...
                 if self.config.walkforward_config.enabled:
                      ...
                      def run_single_asset(asset_cfg):
                           ...
                           if data is None: return {"status": "error", "message": "Data loading failed"}
                           features, target = self.engineer_features(data, asset_cfg)
                           if features is None: return {"status": "error", "message": "Feature engineering failed"}
                           model_path = self.train_model(features, target, asset_cfg)
                           if model_path is None: return {"status": "error", "message": "Model training failed"}
                           # Generate predictions on the *entire* dataset for backtest
                           predictions = self.generate_predictions(features, asset_cfg) # Use original features df
                           if predictions is None: return {"status": "error", "message": "Prediction failed"}
                           signals = self.generate_signals(predictions, data, asset_cfg) # Pass original OHLC data
                           if signals is None: return {"status": "error", "message": "Signal generation failed"}
                           backtest_results = self.backtest_signals(signals, data, asset_cfg) # Pass original OHLC data
                           return {"backtest_summary": backtest_results, "model_path": model_path}
            ...
            for future in concurrent.futures.as_completed(future_to_asset):
                ...
                try:
                    ...
                    logger.exception(f"Asset {asset_symbol} generated an exception during execution: {exc}")
    ...
    def summarize_strategy_results(self, all_results: Dict) -> Dict:
        ...
        for asset, result in all_results.items():
            if isinstance(result, dict):
                 ...
                 summary_data = result.get('walk_forward_summary') or result.get('backtest_summary') or result
                 if isinstance(summary_data, dict) and result.get('status') != 'error':
                      ...
                      overall_metrics_list.append({
                           'asset': asset,
                           'return': summary_data.get('annualized_return_pct', summary_data.get('compounded_return_pct', summary_data.get('total_return_pct'))),
                           'sharpe': summary_data.get('sharpe_ratio', summary_data.get('avg_sharpe')),
                           'max_dd': summary_data.get('max_drawdown_pct', summary_data.get('worst_fold_drawdown_pct')),
                           'win_rate': summary_data.get('win_rate_pct', summary_data.get('avg_win_rate_pct')),
                           'trades': summary_data.get('num_trades', summary_data.get('total_trades'))
                      })
            else:
                 aggregated_summary["assets"][asset] = {"status": "error", "message": "Invalid result format"}


        # Calculate overall portfolio metrics if possible
        if overall_metrics_list:
             metrics_df = pd.DataFrame(overall_metrics_list).set_index('asset').dropna() # Drop assets with NaN metrics for averaging
             if not metrics_df.empty:
                  ...
                  logger.info(f"Overall Average Metrics (across {len(metrics_df)} assets): "
                  ...
                  logger.warning("No valid metrics found across assets for overall summary.")
                  aggregated_summary['overall_summary'] = {"message": "No valid metrics to aggregate"}

        else:
             logger.warning("Could not calculate overall average metrics.")
             aggregated_summary['overall_summary'] = {"message": "No valid asset results"}
    ...
    def plot_strategy_performance(self, overall_summary: Dict, all_results: Dict):
        ...
        try:
            asset_returns = [
                res.get('walk_forward_summary', {}).get('annualized_return_pct') or
                res.get('backtest_summary', {}).get('annualized_return_pct') or 0
                for res in all_results.values() if isinstance(res, dict) and res.get('status') != 'error'
            ]
            ...
            plt.title(f"Distribution of Annualized Returns Across Assets ({self.config.strategy_name})")
            plt.xlabel("Annualized Return (%)")
            plt.ylabel("Frequency")
            plt.grid(alpha=0.3)
            plot_path = os.path.join(self.config.output_dir, "plots", "strategy_return_distribution.png")
...
def run_trading_strategy(config_file: Optional[str] = None) -> Dict:
    ...
    try:
        ...
        if config_file:
            ...
            logger.info("No config file provided, using default strategy configuration.")
        ...
        logger.exception(f"Critical error running trading strategy: {e}") # Log traceback
        return {'error': str(e)}
