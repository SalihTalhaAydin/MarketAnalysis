...
def test_dataloaderconfig_init_custom(mock_makedirs):
    ...
    mock_makedirs.assert_called_once_with(
        "/tmp/cache", exist_ok=True
    )  # Check dir creation attempt
...
def test_dataloaderconfig_to_dict(mock_makedirs, sample_config_dict):
    ...
    if expected_dict.get("default_end_date") is None:
        expected_dict["default_end_date"] = (
...
def mock_config(mock_makedirs, tmp_path):
    ...
    return DataLoaderConfig(
        cache_dir=cache_dir, use_cache=True, rate_limit_pause=0
    )  # No pause for tests
...
def data_loader(mock_config):
    ...
    if loader.cache:
        ...
        loader.cache.get_cached_data = MagicMock(
            return_value=None
        )  # Default: cache miss
...
def test_dataloader_load_data_yahoo_success(
    mock_load_yahoo, data_loader, mock_data_frame
):
...
def test_dataloader_load_data_datareader(mock_load_dr, data_loader, mock_data_frame):
    ...
    data = data_loader.load_data(
        ticker="GDP", interval=dr_source_name, data_source=DataSource.DATAREADER
    )
...
@patch(f"{LOADER_PATH}.load_from_yahoo")
# @patch(f'{LOADER_PATH}.DataCache', MagicMock()) # Removed patch
def test_dataloader_load_data_force_reload(
    mock_load_yahoo, data_loader, mock_data_frame
):
    ...
    data = data_loader.load_data(
        ticker="AAPL", data_source=DataSource.YAHOO, force_reload=True
    )
...
# @patch(f'{LOADER_PATH}.DataCache', MagicMock()) # Removed patch
def test_dataloader_load_multiple_success(
    mock_load_data_method, data_loader, mock_data_frame
):
    ...
    combined_data = data_loader.load_multiple(
        tickers=["AAPL", "MSFT"], column_name="close"
    )
    ...
    mock_load_data_method.assert_has_calls(
        expected_calls, any_order=True
    )  # Use any_order=True if order isn't guaranteed

    assert list(combined_data.columns) == ["AAPL", "MSFT"]
    pd.testing.assert_series_equal(
        combined_data["AAPL"], mock_data_frame["close"], check_names=False
    )
    pd.testing.assert_series_equal(
        combined_data["MSFT"], mock_data_frame["close"] + 10, check_names=False
    )


@patch(f"{LOADER_PATH}.DataLoader.load_data")
# @patch(f'{LOADER_PATH}.DataCache', MagicMock()) # Removed patch
def test_dataloader_load_multiple_partial_fail(
    mock_load_data_method, data_loader, mock_data_frame
):
    ...
    combined_data = data_loader.load_multiple(
        tickers=["AAPL", "GOOG"], column_name="close"
    )
...
def test_dataloader_load_multiple_all_fail(mock_load_data_method, data_loader):
    ...
    combined_data = data_loader.load_multiple(
        tickers=["FAIL1", "FAIL2"], column_name="close"
    )
...
def test_dataloader_apply_rate_limit(mock_time, mock_sleep, mock_config):
    ...
    mock_time.return_value = (
        1004.0  # 2.5s elapsed since last call (1004.0 - 1001.5 = 2.5)
    )
    loader._apply_rate_limit()
    mock_sleep.assert_not_called()
...
def test_load_data_function_basic(
    mock_logger,
    mock_handle_outliers,
    ...
    result_df = load_data(
        ticker=ticker, start_date=start, end_date=end, verbose=False
    )  # verbose=False to avoid basicConfig call issues

    # Check DataLoader was initialized with correct config args
    MockDataLoader.assert_called_once()
    config_arg = MockDataLoader.call_args[0][
        0
    ]  # Get the config object passed to DataLoader.__init__
...
@patch(f"{LOADER_PATH}.handle_outliers")
@patch(f"{LOADER_PATH}.logger")  # Patch the logger instance directly
def test_load_data_function_missing_data(
    mock_logger,
    mock_handle_outliers,
    mock_detect_outliers,
    MockDataLoader,
...
@patch(f"{LOADER_PATH}.handle_outliers")
@patch(f"{LOADER_PATH}.logger")  # Patch the logger instance directly
def test_load_data_function_outliers(
    mock_logger,
    mock_handle_outliers,
    mock_detect_outliers,
    MockDataLoader,
    ...
    outlier_mask = pd.DataFrame(
        False, index=original_df.index, columns=original_df.columns
    )
    ...
    result_df = load_data(
        ...
        start_date="2023-01-01",
        end_date="2023-01-03",
        detect_outliers_method=outlier_method,
        verbose=True,
    )
    ...
    pd.testing.assert_frame_equal(
        call_kwargs_detect["data"], original_df
    )  # Check data passed
    ...
    pd.testing.assert_frame_equal(
        call_kwargs_handle["outliers"], outlier_mask
    )  # outliers arg
    ...
    mock_logger.info.assert_any_call(
        f"Detecting outliers using {outlier_method} method"
    )
...
def test_load_data_function_load_fail(mock_logger, MockDataLoader):
    ...
    result_df = load_data(
        ticker="FAIL", start_date="2023-01-01", end_date="2023-01-03", verbose=False
    )
...
def test_load_data_function_empty_df(mock_logger, MockDataLoader):
    ...
    result_df = load_data(
        ticker="EMPTY", start_date="2023-01-01", end_date="2023-01-03", verbose=False
    )
