...
def test_resample_data_daily_to_weekly(sample_daily_df):
    ...
    assert (
        len(resampled_df) == 3
    )  # 2023-01-01 is week 1, 02-08 is week 2, 09-10 is week 3
...
def test_detect_outliers_unsupported_method(mock_logger, sample_df_for_outliers):
    ...
    mock_logger.error.assert_called_with(
        "Unsupported outlier detection method: unsupported"
    )
...
def outlier_mask(sample_df_for_outliers):
    ...
    mask = pd.DataFrame(
        False,
        index=sample_df_for_outliers.index,
        columns=sample_df_for_outliers.columns,
    )
...
def test_handle_outliers_winsorize(sample_df_for_outliers, outlier_mask):
    ...
    assert (
        handled_df.loc["2023-01-01", "value1"]
        == sample_df_for_outliers.loc["2023-01-01", "value1"]
    )
    assert (
        handled_df.loc["2023-01-01", "value2"]
        == sample_df_for_outliers.loc["2023-01-01", "value2"]
    )
    assert (
        handled_df["string"] == sample_df_for_outliers["string"]
    ).all()  # String column untouched
...
def test_handle_outliers_fillna_interpolate(sample_df_for_outliers, outlier_mask):
    ...
    handled_df = handle_outliers(
        df_copy, outlier_mask, method="fillna", fill_method="interpolate"
    )
...
def test_handle_outliers_fillna_ffill(sample_df_for_outliers, outlier_mask):
    ...
    df_copy = sample_df_for_outliers.copy()
    handled_df = handle_outliers(
        df_copy, outlier_mask, method="fillna", fill_method="ffill"
    )
...
@patch(f"{TRANSFORMATIONS_PATH}.logger")
def test_handle_outliers_unsupported_method(
    mock_logger, sample_df_for_outliers, outlier_mask
):
    """Test handle_outliers with unsupported method."""
    handled_df = handle_outliers(
        sample_df_for_outliers, outlier_mask, method="unsupported"
    )
    mock_logger.error.assert_called_with(
        "Unsupported outlier handling method: unsupported"
    )
...
def test_preprocess_data_standard_df(sample_daily_df):
    ...
    processed_df = preprocess_data(
        df_copy, ticker="ANY"
    )  # Ticker doesn't matter for single index
    ...
    assert (
        processed_df.loc["2023-01-03", "high"] == processed_df.loc["2023-01-02", "high"]
    )
    # Check dtypes
...
def test_preprocess_data_column_warning(mock_logger, sample_daily_df):
    ...
    mock_logger.warning.assert_any_call(
        "Column 'volume' not found for type conversion."
    )
