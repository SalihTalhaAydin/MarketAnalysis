...
def sample_returns_positive():
    ...
    return pd.Series(
        [0.01, 0.02, 0.005, 0.015, 0.01], index=dates, name="PositiveReturns"
    )
...
def sample_returns_mixed():
    ...
    return pd.Series(
        [0.02, -0.01, 0.03, -0.02, 0.01, 0.005, -0.005, 0.015, -0.01, 0.02],
        index=dates,
        name="MixedReturns",
    )
...
def test_calculate_returns_metrics_positive(sample_returns_positive):
    ...
    assert metrics["total_return"] == pytest.approx(
        (1.01 * 1.02 * 1.005 * 1.015 * 1.01) - 1, abs=TOL
    )
    ...
    )  # Sortino is NaN when downside deviation is zero
...
def test_calculate_returns_metrics_mixed(sample_returns_mixed):
    ...
    assert metrics["avg_win"] == pytest.approx(
        sample_returns_mixed[sample_returns_mixed > 0].mean(), abs=TOL
    )
    ...
    assert (
        metrics["max_consecutive_wins"] == 2
    )  # [0.01, 0.005] and [0.015, -0.01, 0.02] -> last two
