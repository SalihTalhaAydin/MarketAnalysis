...
def sample_ohlcv_df():
    ...
    dates = pd.date_range(start="2023-01-01", periods=150, freq='D') # Increased periods for GARCH/ADF
    data = {
        'open': np.linspace(100, 115, 150) + np.random.randn(150) * 0.1, # Adjusted linspace end
...
def mock_pandas_ta(mocker):
    ...
    mock_ta.macd = MagicMock(return_value=pd.DataFrame({
        'MACD_12_26_9': np.random.rand(150),
        'MACDs_12_26_9': np.random.rand(150),
        'MACDh_12_26_9': np.random.rand(150)
    }))
    mock_ta.bbands = MagicMock(return_value=pd.DataFrame({
    ...
    mock_ta.adx = MagicMock(return_value=pd.DataFrame({
        'ADX_14': np.random.rand(150),
        'DMP_14': np.random.rand(150),
        'DMN_14': np.random.rand(150)
    }))
    mock_ta.stoch = MagicMock(return_value=pd.DataFrame({
        'STOCHk_14_3_3': np.random.rand(150),
        'STOCHd_14_3_3': np.random.rand(150)
    }))
    ...
    mock_ta.cdl_engulfing = MagicMock(return_value=pd.Series(np.random.randint(-100, 101, 150)))
    mock_ta.cdl_hammer = MagicMock(return_value=pd.Series(np.random.randint(0, 101, 150)))
...
def mock_advanced_stats(mocker):
    ...
    mock_adfuller = MagicMock(return_value=(0, 0.01, 0, 0, {}, 0)) # Simulate stationary p-value
    ...
    mock_arch_model_instance.fit.return_value.conditional_volatility = pd.Series(np.random.rand(149)*100)
...
def test_calc_indicators_all_basic(sample_ohlcv_df, mock_pandas_ta):
    ...
    result_df = calculate_technical_indicators(sample_ohlcv_df, indicators=None) # None uses defaults
...
def test_calc_indicators_specific(sample_ohlcv_df, mock_pandas_ta):
    ...
    result_df = calculate_technical_indicators(sample_ohlcv_df, indicators=['sma', 'rsi'])
...
def test_calc_indicators_pandas_ta_unavailable(mock_logger, sample_ohlcv_df):
    ...
    mock_logger.error.assert_called_with("pandas-ta not available for calculating indicators")
...
def test_calculate_fractals(sample_ohlcv_df):
    ...
    bull, bear = calculate_fractals(highs, lows, window=3) # Window 3 looks 1 bar each side
...
def test_calculate_efficiency_ratio(sample_ohlcv_df):
    ...
    assert (er.dropna() >= 0).all() and (er.dropna() <= 1).all() # Should be between 0 and 1
...
def test_calculate_market_regime_features(sample_ohlcv_df, mock_pandas_ta, mock_advanced_stats):
