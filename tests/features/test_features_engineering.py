...
@pytest.fixture
def mock_labels(sample_ohlcv_df): # Depend on sample_ohlcv_df to get the correct index length
    ...
    return pd.Series(np.random.choice([-1, 0, 1], size=len(expected_index_for_labeling)), index=expected_index_for_labeling)
...
def mock_calc_indicators(mocker, mock_indicator_df):
    ...
    def side_effect(df, *args, **kwargs):
        ...
        for col in mock_indicator_df.columns:
            if col not in output_df.columns and col in ['SMA_10', 'RSI_14', 'ATRr_10']: # Add only indicator cols
                # Align index before assigning
                output_df[col] = mock_indicator_df[col].reindex(output_df.index)
        return output_df

    return mocker.patch(
        f'{FEATURES_PATH}.calculate_technical_indicators',
...
def mock_get_labels(mocker, mock_labels):
    ...
    return mocker.patch(
        f'{FEATURES_PATH}.get_triple_barrier_labels',
...
def test_engineer_features_basic(sample_ohlcv_df, mock_calc_indicators, mock_get_labels):
...
def test_engineer_features_with_ta(sample_ohlcv_df, mock_calc_indicators, mock_get_labels, mock_indicator_df):
...
def test_engineer_features_triple_barrier(sample_ohlcv_df, mock_calc_indicators, mock_get_labels, mock_indicator_df, mock_labels):
...
def test_engineer_features_directional_target(sample_ohlcv_df, mock_calc_indicators, mock_get_labels):
...
def test_engineer_features_column_sanitization(sample_ohlcv_df, mock_calc_indicators, mock_get_labels):
...
def test_engineer_features_missing_required_cols_barrier(mock_logger, sample_ohlcv_df, mock_calc_indicators, mock_get_labels):
    ...
    mock_logger.error.assert_any_call("Missing required columns for Triple Barrier: ['ATRr_10']")
    mock_get_labels.assert_not_called()

@patch(f'{FEATURES_PATH}.logger')
def test_engineer_features_missing_close_directional(mock_logger, sample_ohlcv_df, mock_calc_indicators, mock_get_labels):
    ...
    assert result_df is None
    mock_logger.error.assert_called_once_with("Missing 'close' column for directional target")

def test_engineer_features_final_nan_handling(sample_ohlcv_df, mock_calc_indicators, mock_get_labels):
    ...
    mock_indicator_df_with_nan['RSI_14'] = [np.nan] * 5 + list(range(15)) # Leading NaNs
