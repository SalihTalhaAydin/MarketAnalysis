...
from market_ml_model.strategy.main import (
    ...
    WalkForwardConfig,
    StrategyConfig,
    MarketRegimeDetector, # Import the class to test
    EnhancedTradingStrategy # Import the main strategy class
)
...
def mock_strategy_dependencies(mocker):
    ...
    mocker.patch(f'{STRATEGY_PATH}.load_model', MagicMock(return_value=(MagicMock(), {}))) # Return dummy model/meta
    ...
    mocker.patch(f'{STRATEGY_PATH}.MODULES_AVAILABLE', True) # Assume modules are available for tests
...
def test_feature_config_dict_conversion():
    ...
    params = {"pattern_features": True, "max_features": 30, "target_type": "directional"}
...
def test_model_config_dict_conversion():
    ...
    params = {"model_type": "xgboost", "optimize_hyperparams": False, "risk_per_trade": 0.01}
...
def test_strategy_config_defaults(mock_makedirs):
    ...
    assert config.output_dir.startswith("strategy_results" + os.path.sep + "enhanced_ml_strategy_")
...
def test_strategy_config_dict_conversion(mock_makedirs, tmp_path):
    ...
    assert config_dict['output_dir'] == run_output_dir # Check specific run dir is saved
...
def test_strategy_config_load(mock_json_load, mock_open_func, mock_makedirs, tmp_path):
    ...
    config_dict_to_load = {
        ...
        'assets': [{'symbol': 'XYZ', 'timeframe': '1d', 'data_source': 'yahoo', 'commission_pct': 0.001, 'slippage_pct': 0.0005, 'min_position_size': 0.01, 'max_position_size': 1.0, 'correlation_group': None}],
        'feature_config': {'technical_indicators': False, 'max_features': 10}, # Partial feature config
        'model_config': {'model_type': 'random_forest'}, # Partial model config
        'walkforward_config': {'enabled': False}, # Partial WF config
        'output_dir': str(tmp_path / "loaded_output" / "loaded_strat_old_run_id"), # Saved output dir
        'parallel_processing': False,
        'random_state': 99,
        'debug_mode': True
    }
    ...
    assert config.output_dir.startswith(str(tmp_path / "loaded_output")) # Base path from loaded config
...
@patch(f'{STRATEGY_PATH}.StandardScaler')
@patch(f'{STRATEGY_PATH}.KMeans')
def test_regime_detector_detect_clustering(MockKMeans, MockStandardScaler, sample_regime_features):
    """Test regime detection using clustering."""
    # Assume sklearn is available
    with patch(f'{STRATEGY_PATH}.SKLEARN_AVAILABLE_FOR_REGIME', True):
        # Setup mocks
        mock_scaler_instance = MagicMock()
        mock_scaler_instance.fit_transform.return_value = np.random.rand(len(sample_regime_features), 3) # Scaled data
        ...
        mock_kmeans_instance.transform.return_value = np.array([[0.5, 0.1, 0.8]]) # Closest to cluster 1
        mock_kmeans_instance.cluster_centers_ = np.random.rand(3, 3) # Dummy centers
        MockKMeans.return_value = mock_kmeans_instance

        detector = MarketRegimeDetector(n_regimes=3, lookback_window=50) # Use lookback < data length
...
def test_regime_detector_insufficient_data(mock_logger, sample_regime_features):
    ...
    regime_info = detector.detect_regime(sample_regime_features)
    assert regime_info is None
    mock_logger.warning.assert_any_call("Not enough data (100) for regime lookback window (200).")
...
def test_regime_detector_missing_features(mock_logger, sample_regime_features):
    ...
    detector = MarketRegimeDetector(regime_features=['volatility_ratio_20_50', 'MISSING_FEATURE'])
    regime_info = detector.detect_regime(sample_regime_features)
    assert regime_info is None
    mock_logger.error.assert_called_with("Missing required features for regime detection: ['MISSING_FEATURE']")
...
def mock_data_loader_instance():
    ...
    loader = MagicMock()
    loader.load_data.return_value = pd.DataFrame({'close': [100, 101, 102]}) # Dummy data
...
def test_enhanced_strategy_init(MockDataLoader, mock_strategy_config):
    ...
    assert isinstance(strategy.regime_detector, MarketRegimeDetector) # Check detector created
...
@patch(f'{STRATEGY_PATH}.DataLoader')
def test_enhanced_strategy_load_data_success(MockDataLoader, mock_strategy_config, mock_data_loader_instance):
...
@patch(f'{STRATEGY_PATH}.DataLoader')
def test_enhanced_strategy_load_data_fail(MockDataLoader, mock_strategy_config, mock_data_loader_instance):
