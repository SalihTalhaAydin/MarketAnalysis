...
def test_trade_manager_update_close_trade(sample_trade_manager):
    ...
    expected_pnl_amount = (trade.exit_price - initial_entry_price) * initial_size # PnL = (Exit - Entry) * Size for long
...
def test_trade_manager_get_trade_summary(sample_trade_manager):
    ...
    assert summary_df.iloc[0]['realized_pnl'] == pytest.approx((112.0 - 110.0) / 110.0, abs=TOL)
...
def test_trade_manager_get_performance_metrics_no_trades(sample_trade_manager):
    ...
    assert metrics['total_return'] == 0.0 # Based on equity curve (only initial capital)
