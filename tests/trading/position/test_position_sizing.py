...
def test_kelly_sizing_provided_stats():
    ...
    win_rate = 0.65
    payoff = 2.5
    expected_kelly = calculate_kelly(win_rate, payoff) # 0.65 - (0.35 / 2.5) = 0.65 - 0.14 = 0.51 -> Half = 0.255
    # Volatility scalar = 1.0
...
def test_kelly_zero_payoff(mock_logger):
    ...
    mock_logger.warning.assert_called_with("Payoff ratio is zero or negative, cannot use Kelly. Defaulting to max risk.") # Correct expected warning
