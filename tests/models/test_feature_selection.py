...
def test_select_features_mutual_info(sample_features_target, mock_selectkbest, mock_mutual_info):
...
def test_select_features_rfe(sample_features_target, mock_rfe, mock_logistic_regression):
    ...
    mock_rfe.assert_called_once_with(estimator=ANY, n_features_to_select=n_select, step=2)
...
def test_select_features_pca(mock_scaler_patch, sample_features_target, mock_pca):
    ...
    mock_scaler_patch.return_value = mock_scaler_instance # Assign to the patched object
    ...
    mock_scaler_instance.fit_transform.assert_called_once_with(X) # Check method called on instance
...
def test_select_features_model_lgbm(sample_features_target, mock_select_from_model, mock_lgbm):
    ...
    mock_select_from_model.assert_called_once_with(ANY, threshold=0.01, max_features=n_select)
...
def test_select_features_model_xgboost(sample_features_target, mock_select_from_model, mock_xgboost):
...
def test_select_features_unsupported_method(mock_logger, sample_features_target):
    ...
    mock_logger.warning.assert_called_with("Unsupported feature selection method: unknown")
...
def test_select_features_sklearn_unavailable(mock_logger, sample_features_target):
    ...
    mock_logger.error.assert_called_with("scikit-learn not available for feature selection")
    pd.testing.assert_frame_equal(X_selected, X)
    assert selected_names == list(X.columns)