...
def mock_preprocessor_object():
    ...
    output_feature_names = ['proc_feat_0', 'proc_feat_1', 'proc_feat_2', 'proc_feat_3', 'proc_feat_4']
    preprocessor.transform.return_value = np.random.rand(10, len(output_feature_names)) # Example transformed shape
...
@patch(f'{PREDICTION_PATH}.open', new_callable=mock_open) # Keep mock_open for file handle check
def test_load_model_from_directory(
    mock_open_func, mock_os_path, mock_joblib_load, mock_json_load,
    mock_model_object, mock_preprocessor_object, sample_metadata
):
    ...
    # Configure mocks
    mock_joblib_load.side_effect = [mock_model_object, mock_preprocessor_object] # Order matters
    ...
    mock_open_func.assert_called_once_with(os.path.join(model_dir, 'training_summary.json'), 'r')
...
def test_load_model_file_not_found_in_dir(mock_os_path):
    ...
    mock_isdir.return_value = True
    mock_isfile.return_value = False
    mock_exists.side_effect = lambda path: not path.endswith('model.pkl') # Model file doesn't exist
...
def test_preprocess_features_with_selection_and_preprocessor(
    ...
    assert list(features_processed.columns) == mock_preprocessor_object.get_feature_names_out()
...
def test_preprocess_features_only_selection(sample_features, sample_metadata):
    ...
    pd.testing.assert_frame_equal(features_processed, sample_features[selected_features])

def test_preprocess_features_only_preprocessor(sample_features, mock_preprocessor_object):
    ...
    mock_preprocessor_object.transform.assert_called_once()
    call_args, _ = mock_preprocessor_object.transform.call_args
    pd.testing.assert_frame_equal(call_args[0], sample_features) # Check input to transform
    assert list(features_processed.columns) == mock_preprocessor_object.get_feature_names_out()
...
def test_preprocess_features_missing_input_feature(sample_features, mock_preprocessor_object):
    ...
    with patch(f'{PREDICTION_PATH}.logger') as mock_logger:
        ...
        mock_logger.warning.assert_any_call("Missing expected features in input data: ['MISSING']. Proceeding with available features.")
        # Check preprocessor was called
        mock_preprocessor_object.transform.assert_called_once() # Should still be called
        call_args, _ = mock_preprocessor_object.transform.call_args
        assert list(call_args[0].columns) == ['feat2_selected', 'feat3_selected'] # Only available passed


# --- Tests for predict_proba ---

def test_predict_proba_success(mock_model_object, sample_features, mock_preprocessor_object, sample_metadata): # Use correct fixture name
    """Test successful probability prediction."""
    probs, names = predict_proba(
        mock_model_object, sample_features, mock_preprocessor_object,
        sample_metadata['selected_features'], sample_metadata['class_names']
    )
...
def test_predict_proba_no_preprocessor(mock_model_object, sample_features): # Use correct fixture name
...
def test_predict_proba_no_predict_proba_method(mock_model_object, sample_features): # Use correct fixture name
...
def test_predict_with_threshold_multiclass(sample_eval_data): # Use correct fixture name
...
def test_model_predictor_init_success(mock_load, mock_model_object, mock_preprocessor_object, sample_metadata): # Add preprocessor to args
...
def test_model_predictor_predict(mock_preprocess, mock_load, mock_model_object, sample_metadata, sample_features, mock_preprocessor_object): # Add preprocessor
    ...
    mock_preprocess.assert_called_once_with(sample_features, predictor.preprocessor, predictor.selected_features)
...
def test_model_predictor_predict_proba(mock_predict_proba, mock_load, mock_model_object, sample_metadata, sample_features, mock_preprocessor_object): # Add preprocessor
    ...
    mock_predict_proba.return_value = (np.random.rand(10, 2), sample_metadata['class_names'])

    predictor = ModelPredictorBase("/fake/path")
    probs, names = predictor.predict_proba(sample_features)

    mock_predict_proba.assert_called_once_with(
        predictor.model, sample_features, predictor.preprocessor,
        predictor.selected_features, predictor.class_names
    )
    assert probs is not None
    assert names == sample_metadata['class_names']
...
def test_predict_with_model_success(MockPredictor, sample_features):
    ...
    pd.testing.assert_frame_equal(call_args[0], features_subset) # Check input to predict_proba
