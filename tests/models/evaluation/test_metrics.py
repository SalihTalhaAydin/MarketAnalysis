...
def sample_eval_data():
    ...
    y_prob = np.array([[0.9, 0.1], [0.2, 0.8], [0.7, 0.3], [0.6, 0.4], [0.8, 0.2],
                       [0.3, 0.7], [0.4, 0.6], [0.9, 0.1], [0.1, 0.9], [0.4, 0.6]])
...
def mock_sklearn_metrics(mocker):
    ...
    mocks = {
        'accuracy_score': mocker.patch(f'{METRICS_PATH}.accuracy_score', return_value=0.7),
        'precision_score': mocker.patch(f'{METRICS_PATH}.precision_score', return_value=0.8),
        ...
        'roc_auc_score': mocker.patch(f'{METRICS_PATH}.roc_auc_score', return_value=0.75),
        'average_precision_score': mocker.patch(f'{METRICS_PATH}.average_precision_score', return_value=0.78),
        'log_loss': mocker.patch(f'{METRICS_PATH}.log_loss', return_value=0.55),
        'confusion_matrix': mocker.patch(f'{METRICS_PATH}.confusion_matrix', return_value=np.array([[4, 1], [2, 3]])),
        'classification_report': mocker.patch(f'{METRICS_PATH}.classification_report', return_value={'0': {}, '1': {}, 'accuracy': 0.7}),
        # Return numpy arrays for curves to allow .tolist() call
        'roc_curve': mocker.patch(f'{METRICS_PATH}.roc_curve', return_value=(np.array([0, 0.2, 1]), np.array([0, 0.7, 1]), np.array([]))),
        'precision_recall_curve': mocker.patch(f'{METRICS_PATH}.precision_recall_curve', return_value=(np.array([0.8, 0.7, 1]), np.array([0.6, 0.8, 0]), np.array([]))),
        'permutation_importance': mocker.patch(f'{METRICS_PATH}.permutation_importance'),
...
def mock_shap(mocker):
    ...
    mocker.patch(f'{METRICS_PATH}.CatBoostClassifier', mock_catboost_module.CatBoostClassifier, create=True)
...
def test_evaluate_classifier_multiclass(sample_eval_data, mock_model, mock_sklearn_metrics):
    ...
    mock_sklearn_metrics['label_binarize'].return_value = np.eye(3)[y_true_multi] # Mock binarized labels
    ...
    # Check mocks
    mock_sklearn_metrics['precision_score'].assert_any_call(ANY, ANY, average='macro', zero_division=0)
    mock_sklearn_metrics['f1_score'].assert_any_call(ANY, ANY, average='weighted', zero_division=0)
...
def test_compute_importance_built_in_rf(sample_eval_data, mock_model):
    ...
    assert list(importance_df['Feature']) == ['feat1', 'feat2'] # Based on mock importances
...
def test_compute_importance_permutation(mock_perm_importance, sample_eval_data, mock_model):
    """Test permutation importance."""
    X, y_true, _, _ = sample_eval_data
    # Setup mock return value for permutation_importance
    mock_perm_importance.return_value = MagicMock(
        importances_mean=np.array([0.5, 0.3]),
        importances_std=np.array([0.05, 0.03])
    )
    importance_df = compute_feature_importance(mock_model, X, y_true, method='permutation')
    mock_perm_importance.assert_called_once()
    assert not importance_df.empty
    assert 'Std' in importance_df.columns
    assert list(importance_df['Feature']) == ['feat1', 'feat2'] # Sorted by importance_mean

@patch(f'{METRICS_PATH}.isinstance') # Patch isinstance directly
def test_compute_importance_shap(mock_isinstance, sample_eval_data, mock_model, mock_shap):
    ...
    def isinstance_side_effect(obj, classinfo):
        ...
        if classinfo in (mock_shap.xgb.XGBClassifier, mock_shap.lgb.LGBMClassifier, mock_shap.CatBoostClassifier):
             return False
...
def test_compute_importance_shap_unavailable_fallback(mock_logger, mock_perm, sample_eval_data, mock_model):
    """Test fallback to permutation when SHAP is unavailable."""
    X, y_true, _, _ = sample_eval_data
    mock_perm.return_value = MagicMock(importances_mean=np.array([0.5, 0.3]), importances_std=np.array([0.05, 0.03]))
    importance_df = compute_feature_importance(mock_model, X, y_true, method='shap')
    mock_logger.warning.assert_any_call("SHAP method requested but SHAP library not available. Falling back to permutation.")
...
def test_plot_confusion_matrix(mock_plotting):
    ...
    mock_plt.savefig.assert_called_once_with('test_cm.png', dpi=300, bbox_inches='tight')
    mock_plt.close.assert_called_once()

def test_plot_feature_importance(mock_plotting):
    """Test feature importance plotting."""
    mock_plt, mock_sns = mock_plotting
    imp_df = pd.DataFrame({'Feature': ['f1', 'f2', 'f3'], 'Importance': [0.5, 0.3, 0.2], 'Std': [0.01, 0.02, 0.01]})
    ...
    mock_plt.savefig.assert_called_once_with('test_fi.png', dpi=300, bbox_inches='tight')
    mock_plt.close.assert_called_once()
...
def test_plot_roc_curve(mock_plotting):
    ...
    mock_plt.savefig.assert_called_once_with('test_roc.png', dpi=300, bbox_inches='tight')
    mock_plt.close.assert_called_once()

def test_plot_pr_curve(mock_plotting):
    """Test Precision-Recall curve plotting."""
    mock_plt, mock_sns = mock_plotting
    plot_precision_recall_curve([1, 0.8, 0.6], [0, 0.5, 1], 0.75, filename='test_pr.png')
    mock_plt.plot.assert_called_once()
    mock_plt.savefig.assert_called_once_with('test_pr.png', dpi=300, bbox_inches='tight')
    mock_plt.close.assert_called_once()
...
def test_generate_model_report(
    mock_open, mock_makedirs, mock_plot_pr, mock_plot_roc, mock_plot_fi,
    mock_plot_cm, mock_compute_fi, mock_evaluate,
    sample_eval_data, mock_model, tmp_path # Use tmp_path for output_dir
):
    ...
    # Mock return values
    train_metrics_mock = {'accuracy': 0.9, 'confusion_matrix': np.eye(2), 'roc_curve': {'fpr': [0,1], 'tpr': [0,1]}, 'roc_auc': 0.9}
    test_metrics_mock = {'accuracy': 0.8, 'confusion_matrix': np.eye(2)*2, 'roc_curve': {'fpr': [0,1], 'tpr': [0,1]}, 'roc_auc': 0.8, 'pr_curve': {'precision': [1,0], 'recall': [0,1]}, 'average_precision': 0.75}
    mock_evaluate.side_effect = [train_metrics_mock, test_metrics_mock]
    importance_df_mock = pd.DataFrame({'Feature': ['f1', 'f2'], 'Importance': [0.6, 0.4]})
