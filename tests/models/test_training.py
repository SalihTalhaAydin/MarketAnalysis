...
from market_ml_model.models.training import create_feature_pipeline, train_classification_model
...
def test_create_pipeline_standard_scaling(
    MockColumnTransformer, MockPipeline, MockOneHotEncoder, MockSimpleImputer,
    MockQuantileTransformer, MockRobustScaler, MockMinMaxScaler, MockStandardScaler,
    sample_feature_df
):
    ...
    assert MockPipeline.call_args_list[0][1]['steps'][0][0] == 'imputer' # First step is imputer
    assert MockPipeline.call_args_list[0][1]['steps'][1][0] == 'scaler' # Second step is scaler
    ...
    assert set(categorical_features_passed) == {'category1', 'category2', 'category_with_nan'}
    MockSimpleImputer.assert_any_call(strategy='most_frequent')
    MockOneHotEncoder.assert_called_once_with(handle_unknown='ignore', sparse_output=False)
...
def test_create_pipeline_unclassified_feature(mock_logger, sample_feature_df):
    ...
    assert "Features in DataFrame but not classified" in mock_logger.warning.call_args[0][0]
...
def test_create_pipeline_sklearn_unavailable(mock_logger, sample_feature_df):
    ...
    assert preprocessor is None
    assert names == []
    mock_logger.error.assert_called_with("scikit-learn not available for feature pipeline")
...
def test_train_classification_model_basic_flow(
    mock_open, mock_makedirs, mock_joblib_dump, mock_train_test_split,
    mock_create_feature_pipeline, mock_select_features, mock_compute_importance,
    mock_evaluate_classifier, mock_optimize_hyperparams, mock_create_model,
    sample_feature_df
):
    """Test the basic successful workflow of train_classification_model."""
    # --- Mock Setup ---
    # Prepare dummy data
    X = sample_feature_df.drop(columns=['category_with_nan']) # Drop col with NaNs for simplicity here
    y = pd.Series(np.random.randint(0, 2, size=len(X)), index=X.index)
    X_train_raw, X_test_raw, y_train, y_test = X.iloc[:8], X.iloc[8:], y.iloc[:8], y.iloc[8:]
    ...
    mock_select_features.return_value = (X_train_raw[selected_features_list], selected_features_list)

    # Preprocessing mock
    mock_preprocessor = MagicMock(spec=ColumnTransformer)
    mock_preprocessor.get_feature_names_out.return_value = ['num__numeric1', 'cat__category1_A', 'cat__category1_B', 'cat__category1_C']
    X_train_processed = pd.DataFrame(np.random.rand(8, 4), index=X_train_raw.index, columns=mock_preprocessor.get_feature_names_out())
    X_test_processed = pd.DataFrame(np.random.rand(2, 4), index=X_test_raw.index, columns=mock_preprocessor.get_feature_names_out())
    mock_preprocessor.fit_transform.return_value = X_train_processed.values # Return numpy array
    mock_preprocessor.transform.return_value = X_test_processed.values # Return numpy array
    mock_create_feature_pipeline.return_value = (mock_preprocessor, selected_features_list) # Return mock preprocessor

    # Model creation / optimization mock
    mock_model = MagicMock()
    mock_model.fit.return_value = None
    mock_optimize_hyperparams.return_value = (mock_model, {'param': 'best'}) # Return fitted model and best params
    mock_create_model.return_value = mock_model # For the case where optimization is off
    ...
    # Importance mock
    mock_importance_df = pd.DataFrame({'Feature': mock_preprocessor.get_feature_names_out(), 'Importance': np.random.rand(4)})
    ...
    assert mock_open.call_count >= 2 # For best_params.json and training_summary.json (and maybe features)