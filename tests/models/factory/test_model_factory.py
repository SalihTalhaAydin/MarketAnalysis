...
from market_ml_model.models.factory.model_factory import MODEL_FACTORY, create_model
...
def mock_imports(mocker):
    ...
    mocker.patch(
        f"{FACTORY_PATH}.RandomForestClassifier",
        return_value=MagicMock(spec=RandomForestClassifier),
    )
    mocker.patch(
        f"{FACTORY_PATH}.GradientBoostingClassifier",
        return_value=MagicMock(spec=GradientBoostingClassifier),
    )
    mocker.patch(
        f"{FACTORY_PATH}.LogisticRegression",
        return_value=MagicMock(spec=LogisticRegression),
    )
    mocker.patch(
        f"{FACTORY_PATH}.VotingClassifier",
        return_value=MagicMock(spec=VotingClassifier),
    )
    ...
    mock_catboost_module.CatBoostClassifier.return_value = (
        MagicMock()
    )  # No spec needed here
    ...
    mocker.patch(
        f"{FACTORY_PATH}.CatBoostClassifier",
    ...
    mock_sequential_instance = MagicMock(
        spec=Sequential if TF_SPEC_AVAILABLE else object
    )
    mocker.patch(
        f"{FACTORY_PATH}.Sequential", return_value=mock_sequential_instance, create=True
    )
    mocker.patch(
        f"{FACTORY_PATH}.Dense",
        MagicMock(spec=Dense if TF_SPEC_AVAILABLE else object),
        create=True,
    )
    mocker.patch(
        f"{FACTORY_PATH}.Dropout",
        MagicMock(spec=Dropout if TF_SPEC_AVAILABLE else object),
        create=True,
    )
    mocker.patch(
        f"{FACTORY_PATH}.LSTM",
        MagicMock(spec=LSTM if TF_SPEC_AVAILABLE else object),
        create=True,
    )
    mocker.patch(
        f"{FACTORY_PATH}.Adam",
        MagicMock(spec=Adam if TF_SPEC_AVAILABLE else object),
        create=True,
    )
...
@pytest.mark.parametrize(
    "model_type, expected_class_path",
    [
...
def test_create_model_unsupported(mock_logger):
    ...
    mock_logger.error.assert_called_with(
        "Unsupported model type: unsupported_model_type"
    )
